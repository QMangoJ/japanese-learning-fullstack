...
export type Block =
	...
	| { type: "title"; jp: string; cn: string; sub?: Bilingual }
	/** A sub-heading inside the material. */
	| { type: "heading"; jp: string; cn: string }
	/** Ordinary running paragraph; `indent` reproduces the 1-character indent. */
	| { type: "paragraph"; jp: string; cn: string; indent?: boolean }
	/** A single standalone line (address line, date line, signature…). */
	| { type: "line"; jp: string; cn: string; align?: "left" | "center" | "right" }
	/** Dialogue line with a speaker label. */
	| { type: "speech"; speaker: string; speakerCn: string; jp: string; cn: string }
