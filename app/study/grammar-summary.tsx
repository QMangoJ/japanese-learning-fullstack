	...
	return (
		<section className="grammar-summary" aria-labelledby={headingId} data-testid="grammar-summary">
			...
			<div className="card grammar-summary__card">
					...
					{summary.rows.map(([form, usage, cn, en], index) => {
						...
						return (
							<article className="grammar-summary__row" key={form}>
								...
								<p>{lang ? en : cn}</p>
								<div className="grammar-summary__actions">
									{example ? <details className="grammar-summary__example">
										<summary>{label("本课例句", "Lesson example")}</summary>
										<div className="grammar-summary__example-body">
				...
				{week === 5 && day === 2 ? <details className="grammar-summary__related">
					<summary>{label("横向对比：其他“完成”表达", "Compare other completion expressions")}</summary>
					...
					<details className="grammar-summary__sources"><summary>{label("等级参考来源", "Level references")}</summary>
